...
const MenuMyDominosBox = () => {
  const dispatch = useDispatch();
  const myDominosBoxMenu = useSelector((state) => state.menu.myDominosBoxMenu || []);
  ...
  const items = myDominosBoxMenu.filter((item) => item.category === selectedSubCategory) ;
...
export default MenuMyDominosBox;
